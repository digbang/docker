...
3. Run `docker pull php:<version>-apache` (example: `docker pull php:8.1-apache`) on each of the versions you want to rebuild.
4. Run `docker-compose up -d --build php` on each modified directory (First `php`, then `php-dev`).
5. Run `docker commit <container> digbang/php:<version>` (example: `docker commit 81-php-1 digbang/php:8.1`).
6. Run `docker commit <container> digbang/php-dev:<version>` (example: `docker commit 81-php-1 digbang/php-dev:8.1`).
